...
async def calculate_chart(data: NatalData):
    lat, lon = await geocode_place(data.place)
...
async def main():
    ...
    for user in users:
        ...
        if not birth or not all(k in birth for k in ["year","month","day","hour","minute","place"]):
            print(f"[REMOVE] {name} missing birth info. Deleting...")
            supabase.table("users").delete().eq("id", user_id).execute()
            continue

        # Skip if chart exists
        if user.get("chart"):
            print(f"[SKIP] {name} already has a chart.")
            continue

        # Generate chart
        try:
            print(f"[GENERATE] Generating chart for {name}...")
            data = NatalData(
                id=user_id,
                name=name,
                year=birth["year"],
            ...
            await calculate_chart(data)
            print(f"[DONE] Chart generated for {name}.")
        except Exception as e:
            print(f"[ERROR] Failed to generate chart for {name}: {e}")
